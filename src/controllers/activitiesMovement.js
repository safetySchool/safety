const Movement = require('../models/activities_movement');
...
exports.getDocumentSafetySchoolValidation = async (request, h) => {
    try {
...
exports.create = async (request, h) => {
    try {
        const documentData = {
            activity: request.payload.referenceId,
    ...
    try {
        ...
        if (files) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                //SAVE IN DB
                console.log({ name, fileType, relativePath });
