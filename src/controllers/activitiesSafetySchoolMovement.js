const MovementSafetySchool = require('../models/activitiesSafetySchool_movement');
...
exports.get = async (request, h) => {
    try {
        const activitySafetySchoolId = request.params.id;
        const institution = request.auth.credentials.institution;
        const movement = await MovementSafetySchool.find({
            institution: institution,
            activitySafetySchool: activitySafetySchoolId,
            active: true
...
exports.getLastMovementDate = async (request, h) => {
    try {
        const activityId = request.params.activitySafetySchool;
        const institution = request.auth.credentials.institution;
        const lastMovement = await MovementSafetySchool.find({
            institution: institution,
            activitySafetySchool: activityId,
            active: true
        }).exec();
        ...
        if (lastMovement.length === 0) {
            ...
            return await MovementSafetySchool.findOne({
                _id: lastMovementId
            }).exec();
        }
    } catch (error) {
...
exports.getActivitiesMovementValidation = async (request, h) => {
    try {
        ...
        const activityId = request.params.referenceId;
        const lastValidation = await MovementSafetySchool.find({
            institution: institution,
            activitySafetySchool: activityId,
            active: true
        }).exec();
...
exports.getFilesByActivitiesMovementId = async (request, h) => {
    try {
        const document = await MovementSafetySchool.findById(request.query._id).exec();
        if (!document) {
            return { message: 'Registro no encontrado' };
        }
...
exports.getById = async (request, h) => {
    try {
        const document = await MovementSafetySchool.findById(request.params._id).exec();
        if (!document) {
            return { message: 'Registro no encontrado' };
        }
        return document;
...
exports.create = async (request, h) => {
    try {
        const institution = request.auth.credentials.institution;
        const documentData = {
            activitySafetySchool: request.payload.referenceId,
        ...
        const document = await MovementSafetySchool.create(documentData);
        return document._id;
    } catch (error) {
        console.log(error);
...
exports.update = async (request, h) => {
    try {
        const document = await MovementSafetySchool.findById(request.payload._id).exec();
        if (!document) {
...
exports.updateDate = async (request, h) => {
    try {
        const document = await MovementSafetySchool.findById(request.payload.people).exec();
        if (!document) {
            return { err: 'Registro no encontrado' };
        }
        document.date = request.payload.date || document.date;
...
exports.upload = async (request, h) => {
    try {
        ...
        const document = await MovementSafetySchool.findById(_id).exec();
...
exports.getActivitiesSafetySchoolMovementValidation = async (request, h) => {
    try {
        const activityId = request.params.activitySafetySchool;
        const institution = request.auth.credentials.institution;
        const lastValidation = await MovementSafetySchool.find({
    ...
    try {
        ...
        if (files) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                const movementDocument = await MovementSafetySchool.findById(_id).exec();
