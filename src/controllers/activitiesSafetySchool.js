const ActivitySafetySchool = require('../models/activitiesSafetySchool');
...
exports.list = async (request, h) => {
    try {
        return await ActivitySafetySchool.find();
    } catch (error) {
        console.log(error);
...
exports.listByInstitutionId = async (request, h) => {
    try {
        return await Activity.find({ active: true, institution: request.params.institution }).populate('institution', { name: 1 }).exec();
    } catch (error) {
        console.log(error);
        Boom.badImplementation('Internal Error');
    }
